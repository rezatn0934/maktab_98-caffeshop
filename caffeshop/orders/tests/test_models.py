...
class TestOrdersModels(TestCase):
    ...
    def setUp(self):
        ...
        self.order_item1 = Order_detail.objects.create(
            order=self.order,
            product=Product.objects.create(category=Category.objects.create(name='Foods'), name='rice',
                                           description='for test', price=8.00),
            quantity=1,
        )
        self.order_item2 = Order_detail.objects.create(
            order=self.order,
            product=Product.objects.create(category=Category.objects.create(name='FastFood'), name='hamburger',
                                           description='for test', price=6.00),
            quantity=3,
        )
    ...
    def test_get_order_details(self):
        ...
        order_item = Order_detail.objects.create(
            order=order,
            product=Product.objects.create(category=Category.objects.create(name='Drinks'), name='Tea',
                                           description='for test', price=5.00),
            quantity=1,
            price=10.00)

    def test_get_order_items(self):
        order = Order.objects.create(payment='P', status='A', phone_number='09152593858',
                                     table_number=Table.objects.create(
                                         name='orchid', Table_number=4, occupied=True))
        order_items = Order_detail.objects.create(order=order,
                                                  product=Product.objects.create(
                                                      category=Category.objects.create(name='Drinks'), name='Tea',
                                                      description='for test', price=5.00),
    ...
    def test_str(self):
        self.assertEquals(str(self.order), 'Order105')
    ...
    def test_total_order_price(self):
        order = Order.objects.create(
            ...
            status='A',
            phone_number='09152593858',
            table_number=Table.objects.create(name='lavender', Table_number=13, occupied=True),
            order_date='2023-03-03',
        )
        order_detail = Order_detail.objects.create(
            order=order,
            product=Product.objects.create(category=Category.objects.create(name='hotdrinks'), name='Coffee',
                                           price=10.00, description='ffffff'),
            quantity=1,
            price=100
        )
        order_detail1 = Order_detail.objects.create(
            order=order,
            product=Product.objects.create(category=Category.objects.create(name='colddrinks'), name='soda',
                                           price=30.00, description='cold drink'),
            quantity=3,
            price=30
        )
    ...
    def test_has_no_quantity(self):
        ...
        order_detail2 = Order_detail.objects.create(
            order=order1,
            product=Product.objects.create(category=Category.objects.create(name='pizzas'), name='peperoni',
                                           price=40.00, description='peperoni pizza'),
            quantity=0,
            price=100
        )
        order_detail3 = Order_detail.objects.create(
            order=order1,
            product=Product.objects.create(category=Category.objects.create(name='traditional'), name='ghorme',
                                           price=80.00, description='food'),
            quantity=0,
            price=30
        )
    ...
    def test_save(self):
        self.order = Order.objects.create(
            table_number=Table.objects.create(name='pansy', Table_number=78, occupied=True))
        self.order.save()
...
class TestOrderDetailsModel(TestCase):
    def test_str(self):
        self.order = Order.objects.create(
            payment='P',
            status='A',
        ...
        self.assertEquals(str(self.order_item1), 'Order Details ID: 107')
